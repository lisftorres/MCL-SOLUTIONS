...
import { Plus, Edit2, Trash2, Shield, Mail, Building, User as UserIcon, X, Lock } from 'lucide-react';
...
const UserManager: React.FC<UserManagerProps> = ({ users = [], clubs = [], userPasswords = {}, onAddUser, onEditUser, onDeleteUser }) => {
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {users.map(user => {
          if (!user) return null;
          return (
            <div key={user.id} className="bg-brand-light rounded-2xl border border-gray-700 overflow-hidden shadow-xl group transition-all hover:border-brand-yellow/40 flex flex-col h-full">
              ...
              <div className="p-5 bg-brand-dark/20 min-h-[100px] flex-1">
                <div className="flex flex-wrap gap-2 justify-center">
                  ...
                  {(!user.clubIds || user.clubIds.length === 0) && <span className="text-[10px] text-gray-600 font-black uppercase tracking-widest italic">Accès Global</span>}
                </div>
              </div>
            </div>
          );
      ...
      {/* MODAL UTILISATEUR - FULL BLACK & BOLD */}
      {showModal && (
          ...
          <div className="bg-white w-full max-w-lg rounded-2xl shadow-2xl overflow-hidden animate-fade-in-up flex flex-col max-h-[95vh]">
            ...
            <form onSubmit={handleSubmit} className="p-8 space-y-6 overflow-y-auto custom-scrollbar">
              <div className="space-y-1">
                <label className="block text-[11px] font-black text-black uppercase tracking-widest">Nom complet</label>
                <input type="text" required className="w-full bg-gray-50 border border-gray-300 rounded-xl p-4 text-black font-black outline-none focus:ring-2 focus:ring-brand-yellow transition-all placeholder:text-gray-400" value={formData.name} onChange={e => setFormData({...formData, name: e.target.value})} placeholder="Ex: MARIE CIAPPA" />
              </div>
              <div className="space-y-1">
                <label className="block text-[11px] font-black text-black uppercase tracking-widest">Adresse Email</label>
                <input type="email" required className="w-full bg-gray-50 border border-gray-300 rounded-xl p-4 text-black font-black outline-none focus:ring-2 focus:ring-brand-yellow transition-all placeholder:text-gray-400" value={formData.email} onChange={e => setFormData({...formData, email: e.target.value})} placeholder="mariea.ciappa@gmail.com" />
              </div>
              <div className="space-y-1">
                <label className="block text-[11px] font-black text-black uppercase tracking-widest">Mot de passe</label>
                <div className="relative">
                   <Lock className="absolute left-4 top-1/2 -translate-y-1/2 text-gray-400" size={18} />
                   <input type="text" required={!isEditing} className="w-full bg-gray-50 border border-gray-300 rounded-xl pl-12 pr-4 py-4 text-black font-black outline-none focus:ring-2 focus:ring-brand-yellow transition-all placeholder:text-gray-400" value={password} onChange={e => setPassword(e.target.value)} placeholder={isEditing ? "Modifier si nécessaire..." : "123456"} />
                </div>
              </div>
              <div className="space-y-1">
                <label className="block text-[11px] font-black text-black uppercase tracking-widest">Accès / Rôle</label>
                <select className="w-full bg-gray-50 border border-gray-300 rounded-xl p-4 text-black font-black outline-none focus:ring-2 focus:ring-brand-yellow transition-all cursor-pointer" value={formData.role} onChange={e => setFormData({...formData, role: e.target.value as UserRole})}>
              ...
              <div className="space-y-2">
                <label className="block text-[11px] font-black text-black uppercase tracking-widest">Clubs autorisés</label>
                <div className="space-y-2 bg-gray-50 p-4 rounded-xl border border-gray-200 max-h-40 overflow-y-auto custom-scrollbar shadow-inner">
                  {clubs.map(club => (
