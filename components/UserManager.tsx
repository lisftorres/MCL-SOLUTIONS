...
const UserManager: React.FC<UserManagerProps> = ({ users, clubs, userPasswords, onAddUser, onEditUser, onDeleteUser, onDeletePassword }) => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (isEditing && formData.id) {
      ...
      // On exécute l'ajout immédiatement pour la réactivité
          ...
          <div className="bg-white w-full max-w-lg rounded-2xl shadow-2xl overflow-hidden animate-fade-in-up flex flex-col max-h-[95vh]">
            ...
            <form onSubmit={handleSubmit} className="p-8 space-y-6 overflow-y-auto custom-scrollbar">
              <div className="space-y-1">
                ...
                <input type="text" required className="w-full bg-gray-50 border border-gray-200 rounded-xl p-4 text-black font-black outline-none focus:ring-2 focus:ring-brand-yellow transition-all" value={formData.name} onChange={e => setFormData({...formData, name: e.target.value})} placeholder="Ex: Jean Martin" />
              </div>
              <div className="space-y-1">
                <label className="block text-[10px] font-black text-gray-400 uppercase tracking-widest">Adresse Email</label>
                <input type="email" required className="w-full bg-gray-50 border border-gray-200 rounded-xl p-4 text-black font-black outline-none focus:ring-2 focus:ring-brand-yellow transition-all" value={formData.email} onChange={e => setFormData({...formData, email: e.target.value})} placeholder="jean.martin@gmail.com" />
              </div>
              <div className="space-y-1">
                ...
                <div className="space-y-2 bg-gray-50 p-4 rounded-xl border border-gray-100 max-h-40 overflow-y-auto custom-scrollbar shadow-inner">
                  {clubs.map(club => (
                    <label key={club.id} className="flex items-center space-x-3 cursor-pointer hover:bg-white p-3 rounded-xl transition-all shadow-sm border border-transparent hover:border-gray-200 group">
                      ...
                      <span className="text-[11px] font-black text-brand-dark uppercase tracking-tighter group-hover:text-black">{club.name}</span>
                    </label>
                  ))}
                </div>
              </div>
