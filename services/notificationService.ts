
import { requestChatCompletion } from "./omnirouteClient";
import { Ticket, Club, User } from "../types";

/**
 * Service de notification pour MCL Solutions.
 * Utilise OmniRoute (passerelle IA) pour rédiger des communications techniques précises.
 */
export const notificationService = {
  /**
   * Génère un contenu d'e-mail professionnel via OmniRoute et simule l'envoi.
   */
  sendTicketEmail: async (ticket: Ticket, club: Club | undefined, creator: User | undefined) => {
    try {
      const content = await requestChatCompletion(
        `Rédige un e-mail de notification de maintenance court et professionnel.
        Détails du problème : ${ticket.description}.
        Lieu : ${club?.name} - ${ticket.space}.
        Urgence : ${ticket.urgency}.
        Métier concerné : ${ticket.trade}.
        Déclarant : ${creator?.name}.`,
        {
          systemPrompt: `Tu réponds uniquement avec un objet JSON de la forme :
          {
            "subject": string, // Sujet de l'email
            "body": string, // Corps de l'email formaté
            "recipientRole": string // Rôle visé (ex: Technicien Plomberie)
          }`
        }
      );

      const emailContent = JSON.parse(content);

      // Simulation d'envoi (Log console pour le développeur)
      console.log("%c [EMAIL SENT] ", "background: #F7CE3E; color: #373F47; font-weight: bold;", {
        to: "maintenance-team@mclsolutions.fr",
        ...emailContent
      });

      return emailContent;
    } catch (error) {
      console.error("Erreur génération email notification:", error);
      return null;
    }
  }
};
